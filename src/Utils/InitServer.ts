...
export const initServer = async () => {
    ...
    if (config.autoMirror) {
        ...
        const channelsToAdd = guild.channels.filter(
            channel =>
                !config.target_channels.find(
                    configChannel =>
                        configChannel.target_channel_id === channel.id,
                ) && channel.type !== 15,
        );
        ...
        channelsToAdd.sort((a, b) => {
            ...
            if (
                channelTypes.includes(a.type) &&
                channelTypes.includes(b.type)
            ) {
        ...
        for (const channel of channelsToAdd) {
            ...
            if (!canSeeChannel) {
                Log.info(
                    `Skipping channel ${channel.name} as user cannot see it`,
                );
                continue;
            }
